...
func handleDiscordInteraction(b *Bot, w http.ResponseWriter, r *http.Request) {
	...
	publicKey := b.cfg.Discord.PublicKey
	if publicKey == "" {
	...
	if !verifyDiscordSignature(publicKey, sig, ts, body) {
		log.Warn("discord interactions: invalid signature", "ip", httputil.ClientIP(r))
	...
	case InteractionTypePing:
		...
		json.NewEncoder(w).Encode(InteractionResponse{Type: InteractionResponsePong})
		return

	case InteractionTypeMessageComponent:
		handleComponentInteraction(ctx, b, w, &interaction)
		return

	case InteractionTypeModalSubmit:
		handleModalSubmit(ctx, b, w, &interaction)
		return
...
func handleComponentInteraction(ctx context.Context, b *Bot, w http.ResponseWriter, interaction *Interaction) {
	...
	response := handleBuiltinComponent(ctx, b, data, userID)
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(response)
}

// handleModalSubmit processes modal form submissions.
func handleModalSubmit(ctx context.Context, b *Bot, w http.ResponseWriter, interaction *Interaction) {
	var data InteractionData
	if err := json.Unmarshal(interaction.Data, &data); err != nil {
...
func handleBuiltinComponent(ctx context.Context, b *Bot, data InteractionData, userID string) InteractionResponse {
	customID := data.CustomID
	...
	if strings.HasPrefix(customID, "gate_approve:") {
		...
		if b.approvalGate != nil {
			b.approvalGate.handleGateCallback(reqID, true)
		}
	...
	if strings.HasPrefix(customID, "gate_reject:") {
		...
		if b.approvalGate != nil {
			b.approvalGate.handleGateCallback(reqID, false)
		}
		return InteractionResponse{
			Type: InteractionResponseUpdateMessage,
			Data: &InteractionResponseData{
	...
	if strings.HasPrefix(customID, "approve:") {
		...
		audit.Log(b.cfg.HistoryDB, "discord.component.approve", "discord",
			fmt.Sprintf("task=%s user=%s", taskID, userID), "")
		return InteractionResponse{
			Type: InteractionResponseUpdateMessage,
			Data: &InteractionResponseData{
				Content: fmt.Sprintf("dtypes.Task `%s` approved by <@%s>.", truncate(taskID, 8), userID),
			},
		}
	}
	...
	if strings.HasPrefix(customID, "reject:") {
		...
		audit.Log(b.cfg.HistoryDB, "discord.component.reject", "discord",
			fmt.Sprintf("task=%s user=%s", taskID, userID), "")
		return InteractionResponse{
			Type: InteractionResponseUpdateMessage,
			Data: &InteractionResponseData{
				Content: fmt.Sprintf("dtypes.Task `%s` rejected by <@%s>.", truncate(taskID, 8), userID),
			},
		}
	}
