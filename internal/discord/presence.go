...
func (b *Bot) SetTyping(ctx context.Context, channelRef string) error {
	...
	url := APIBase + fmt.Sprintf("/channels/%s/typing", channelRef)
	req, err := http.NewRequestWithContext(ctx, "POST", url, nil)
	if err != nil {
		return err
	}
	req.Header.Set("Authorization", "Bot "+b.cfg.Discord.BotToken)
	resp, err := b.api.HTTPClient.Do(req)
	if err != nil {
		return err
	}
	resp.Body.Close()
...
func (b *Bot) PresenceName() string { return "discord" }
