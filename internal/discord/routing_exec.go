...
func (b *Bot) sendRouteResponse(channelID string, route *dtypes.RouteResult, result dtypes.TaskResult, task dtypes.Task, skipOutput bool, replyMsgID string) {
	color := 0x57F287
	...
	if !skipOutput {
		...
		if strings.TrimSpace(output) == "" && result.Status == "success" {
			parts := []string{"dtypes.Task completed successfully."}
		...
		if result.Status == "success" && strings.TrimSpace(output) != "" && b.cfg.BaseDir != "" {
			outDir := filepath.Join(b.cfg.BaseDir, "outputs")
	...
	b.sendEmbedReply(channelID, replyMsgID, Embed{
		...
		Fields: []EmbedField{
			...
			{Name: "今日 Token", Value: formatTokenField(todayIn, todayOut, b.cfg.CostAlert.DailyTokenLimit), Inline: true},
		},
		Footer:    &EmbedFooter{Text: fmt.Sprintf("dtypes.Task: %s", task.ID[:8])},
