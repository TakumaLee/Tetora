...
func cmdInit() {
	...
	case 1: // Discord
		...
		fmt.Println("    4. Bot token → Bot page → Reset Token → copy")
		fmt.Println("    5. Enable Message Content Intent on the Bot page")
		fmt.Println("    6. OAuth2 → URL Generator → scope: bot → invite to server")
		fmt.Println("    7. Channel ID → enable Developer Mode in Discord settings,")
		fmt.Println("       then right-click a channel → Copy Channel ID\033[0m")
		fmt.Println()
