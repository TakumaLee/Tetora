  ...
  function triggerAutoUpdate() {
    ...
    fetch('/api/war-room/autoupdate/trigger', { method: 'POST' })
      .then(function(r) {
        // 202 Accepted = fire-and-forget scheduled; 200 also acceptable for back-compat.
        if (!r.ok && r.status !== 202) throw new Error('HTTP ' + r.status);
        return r.json().catch(function() { return {}; });
      })
      .then(function() { pollAutoUpdateStatus(btn); })
      .catch(function(err) {
        alert('autoupdate 觸發失敗：' + err.message);
  ...
  function pollAutoUpdateStatus(btn) {
    ...
    function tick() {
      ...
      fetch('/api/war-room/autoupdate/meta')
        ...
        .then(function(m) {
          ...
          // Transition running=true → running=false signals completion.
          if (sawRunning && !m.running) { finish(); return; }
          if (attempts >= maxAttempts) { finish(true); return; }
    ...
    // Short delay so the cron engine has scheduled the goroutine by first poll.
    setTimeout(tick, 500);
  }
