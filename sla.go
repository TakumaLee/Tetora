...
// SLAConfig configures per-role SLA monitoring.
type SLAConfig struct {
	Enabled       bool                  `json:"enabled,omitempty"`
	Roles         map[string]RoleSLACfg `json:"roles,omitempty"`
...
// RoleSLACfg defines SLA thresholds for a single role.
type RoleSLACfg struct {
...
// SLAMetrics holds computed SLA metrics for a single role.
...
// SLAStatus holds SLA metrics plus violation status for a role.
...
func initSLADB(dbPath string) {
	// Add role column to job_runs if not exists.
	migrate := `ALTER TABLE job_runs ADD COLUMN role TEXT DEFAULT '';`
...
CREATE INDEX IF NOT EXISTS idx_sla_checks_role ON sla_checks(role);
...
// querySLAMetrics computes SLA metrics for a single role over a time window.
func querySLAMetrics(dbPath, role string, windowHours int) (*SLAMetrics, error) {
	if dbPath == "" {
		return &SLAMetrics{Role: role}, nil
	}
	if windowHours <= 0 {
		windowHours = 24
	}
...
func querySLAStatusAll(cfg *Config) ([]SLAStatus, error) {
	...
	for _, role := range roles {
		...
		// Check thresholds.
		if roleCfg, ok := cfg.SLA.Roles[role]; ok {
...
func checkSLAViolations(cfg *Config, notifyFn func(string)) {
	...
	for role, roleCfg := range cfg.SLA.Roles {
		m, err := querySLAMetrics(cfg.HistoryDB, role, windowHours)
		if err != nil {
			logWarn("SLA check query failed", "role", role, "error", err)
			continue
		}
...
func recordSLACheck(dbPath string, r SLACheckResult) {
	...
	sql := fmt.Sprintf(
		`INSERT INTO sla_checks (role, checked_at, success_rate, p95_latency_ms, violation, detail)
		 VALUES ('%s', '%s', %f, %d, %d, '%s')`,
		escapeSQLite(r.Role),
...
// querySLAHistory returns recent SLA check results for a role.
func querySLAHistory(dbPath, role string, limit int) ([]SLACheckResult, error) {
	...
	sql := fmt.Sprintf(
		`SELECT role, checked_at, success_rate, p95_latency_ms, violation, detail
